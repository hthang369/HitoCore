...
namespace HitoAppCore.DataGrid
{
    public class GridControl : GridLayout, IDisposable, IServiceProvider
    {
        ...
        public const double DefaultRowHeight = 40;
        ...
        public GridControl()
        {
            ...
            headerFilter.textChanged += HeaderFilter_textChanged;
        ...
        private void HeaderFilter_textChanged(object sender, TextChangedEventArgs e)
        {
            ...
            {
                listViewSource.ItemsSource = GetDataSource(view.FieldNameSelected, this.dataSource, e.NewTextValue);
            }
        }
        private IEnumerable<object> GetDataSource(List<string> lstColumns, List<object> lst, string key)
        {
            //for (int i = 0; i < lstColumns.Count; i++)
            //{
            //    List<object> list = lst.Where(x => Convert.ToString(ObjectUtils.GetPropertyValue(x, lstColumns[i])).Contains(key)).ToList();
